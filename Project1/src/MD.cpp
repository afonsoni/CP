...
double Potential()
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        for (int j = i + 1; j < N; j++)
        {
            ...
            double term2 = quot * quot * quot * 1.;
            double term1 = term2 * quot * quot * quot * 1.;
...
void computeAccelerations()
{
    ...
    { // set all accelerations to zero
        ...
        for (int j = i + 1; j < N; j++)
        {
            ...
            double rSqd7 = rSqd4 / (rSqd * rSqd * rSqd);
...
double VelocityVerlet(double dt, int iter, FILE *fp)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        r[0][i] += v[0][i] * dt + half_dt * a[0][i] * dt;
        r[1][i] += v[1][i] * dt + half_dt * a[1][i] * dt;
        r[2][i] += v[2][i] * dt + half_dt * a[2][i] * dt;
