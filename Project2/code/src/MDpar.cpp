...
{   
    ...
    // Parallelize the loop using OpenMP for improved performance
    // Calculate forces and potential energy between each pair of atoms
    #pragma omp parallel for reduction(+:Pot) reduction(+:a[:MAXPART][:3]) private(j, rSqd, rSqd4, rSqd7, rij, f) schedule(dynamic, 40)
    for (i = 0; i < N-1; i++) 
    {  
        for (j = i + 1; j < N; j++)
        {
